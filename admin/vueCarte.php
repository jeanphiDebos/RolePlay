<div id="Corps" class="adminCarte">
    <div class="row clearfix listeAction">
            ...
            <div class="input-group">
                ...
                <select class="form-control" id="listeCarte">
                    <option value="" class="defautValue"
                            <?php if (empty($_SESSION['carte'])){ ?>selected="selected"<?php } ?>>selectionner une carte
                    </option>
                    <?php foreach ($listeCartes as $uneCarte) { ?>
                        <option value="<?php echo $uneCarte['id']; ?>" class="uneCarteOption"
                            <?php if (isset($_SESSION['carte']) && $_SESSION['carte'] == $uneCarte['id']){ ?>selected="selected"<?php } ?>><?php echo $uneCarte['nom']; ?></option>
                    <?php } ?>
                </select>
            </div>
        </div>
        ...
        <div class="col-md-1 column">
            <input type="checkbox" id="activerAfficher" data-on-color="success" data-off-color="danger"
                   data-label-text="Appel">
        </div>
        <div class="col-md-1 column">
...
<script>
    $(document).ready(function () {
        var IDCarte = "<?php if (isset($_SESSION['carte'])) {
            echo $_SESSION['carte'];
        }?>";
        ...
        evenButtonAddCarteClick("<?php echo $pathUploadFichier; ?>", "#addCarte");
        evenSelectCarteChange("#listeCarte");
